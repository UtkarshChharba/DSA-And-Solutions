...
pair<TreeNode<int>*,int> max_sum(TreeNode<int> *root){
    pair<TreeNode<int>*,int> max=mp(root,root->data);
    loop(i,0,root->children.size()){
        max.ss+=root->children[i]->data;
    }
    loop(i,0,root->children.size()){
        pair<TreeNode<int>*,int> children_max=max_sum(root->children[i]);
        if(max.ss<children_max.ss){
            max=children_max;
        }
    }
    return max;
...
int32_t main(){
    ...
    cout<<max_sum(root).ff->data<<endl;
