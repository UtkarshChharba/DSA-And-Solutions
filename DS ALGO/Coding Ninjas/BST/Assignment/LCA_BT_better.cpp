...
int LCA_finder(Binary_Tree<int>*root,int data1,int data2){
    if(!root){
        return -1;
    }
    ...
    int left_lca=LCA_finder(root->left,data1,data2);
    int right_lca=LCA_finder(root->right,data1,data2);
    if(left_lca!=-1 And right_lca!=-1){
        return root->data;
    }
    if(min(left_lca,right_lca)==-1){
        return max(left_lca,right_lca);
    }
    return -1;
...
int LCA_data=LCA_finder(root,data1,data2);
cout<<LCA_data<<endl;
return 0;
