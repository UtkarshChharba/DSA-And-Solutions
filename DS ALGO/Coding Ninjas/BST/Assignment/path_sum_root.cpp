...
void path_sum(Binary_Tree<int>*root,int k,vi &v,int sum=0){
    if(!root){
        return;
    }
    if(root->data+sum>k){
    ...
    v.pb(root->data);
    if(root->left==NULL And root->right==NULL And sum+root->data==k){
        print_vector(v);
    }
    path_sum(root->left,k,v,sum+root->data);
    path_sum(root->right,k,v,sum+root->data);
    v.pop_back();
    return;
