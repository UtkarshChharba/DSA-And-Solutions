...
bool finder(Binary_Tree<int>*root,int a){
    if(!root){
        return false;
    }
    if(root->data==a){
...
int LCA_BST(Binary_Tree<int>*root,int a,int b){
    if(!root){
        return -1;
    }
    ...
    if(a<root->data And b>=root->data){
        bool for_a=finder(root,a);
        bool for_b=finder(root,b);
        if(for_a And for_b){
            return root->data;
        }
        if(for_a){
