...
void node_depth(Binary_Tree<int>*root,int depth){
    if(!root){
        return ;
    }
    if(!depth){
    ...
    node_depth(root->left,depth-1);
    node_depth(root->right,depth-1);
}
int node_distance(Binary_Tree<int>*root,int k,int data){
    if(!root){
        return -1;
    }
    if(root->data==data){
    ...
    int left=node_distance(root->left,k,data);
    int right=node_distance(root->right,k,data);
    if(left!=-1){
        int distance=1+left;
        if(distance==k){
            cout<<root->data<<endl;
        ...
        if(distance<k){
           node_depth(root->right,k-distance-1);
           return distance;
        }
        return -1;
    }
    if(right!=-1){
       int distance=1+right;
        if(distance==k){
            cout<<root->data<<endl;;
        }
        if(distance<k){
            node_depth(root->left,k-distance-1);
            return distance;
        }
        return -1;
    }
