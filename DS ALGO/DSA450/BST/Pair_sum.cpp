...
bool check(Binary_Tree<int>*root,int sum,Binary_Tree<int>*cur_node){
    if(!root){
        return 0;
    }
    if(root->data==sum && root!=cur_node){return 1;}
    return check(root->left,sum,cur_node)||check(root->right,sum,cur_node);

}
void path_sum(Binary_Tree<int>*cur_node,int k,Binary_Tree<int>*root){
    if(!cur_node){
        return ;
    }
    if(k>cur_node->data  && check(root,k-cur_node->data,cur_node)){
        cout<<cur_node->data<<" "<<k-cur_node->data<<endl;
        cur_node->data=INT_MAX;
    }
    path_sum(cur_node->left,k,root);
    path_sum(cur_node->right,k,root);
    return;
